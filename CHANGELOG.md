...
- One shared refresh-and-retry on 401/403, with a `SESSION_REVOKED` short-circuit
  and a `sessionExpired` event. Coded input errors (`CSRF_INVALID`, ...) and the
  pre-session verify routes are never retried, a logout wins over a refresh in
  flight, and a network failure never reads as signed out. `client.fetch` shares it without patching the
  global `fetch`.
...
- `@awesome-lang-auth/react/server`: `hasRole`, `SERVER_SNAPSHOT` and the types, callable from React Server Components.
